...
class Board:
    ...
    def __init__(self):
        ...
        self.delta = [(0, 1), (0, -1), (1, 0), (-1, 1), (1, -1), (1, 1), (-1, -1)]

    def _get_empty_board(self) -> List[List[int]]:
        # TODO Add sentinel element, like -1
    ...
    def put_chip(self, col: int, player: str) -> None:
    ...
    def check_win_cell(self, row: int, col: int, player: str) -> None:
        ...
        for delta_row, delta_col in self.delta:
            # print(row, col, delta_row, delta_col, player)
            if self._check_win(row, col, delta_row, delta_col, player):
                raise WinGame(f"Player {player} is win!")

    def _check_win(self, row: int, col: int, delta_row: int, delta_col: int, player: str) -> bool:
